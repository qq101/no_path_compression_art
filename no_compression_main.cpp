#include "no_path_compression.cpp"
#include "iostream"
#include "fstream"
using namespace std;
...
//Insert
   ...
   while (fib_stream.getline(buffer, 256)) {
      ...
      port = atoi(id);
      line++;
      insert(tree, &tree, (uint8_t*)name, port + 1, len + 1);
   }
...
//Lookup
   ...
   while (trace_stream.getline(buffer, 256)) {
      ...
      buffer[len] = '\0';
      line++;
      uintptr_t result = lookup(tree, (uint8_t*)buffer, len + 1, len + 1);

      if (result) {
         cout << "port=" << result-1 << endl;
         count++;
      }
   }
   cout << "lookup line:" << line << "  matched num:" << count << " , time:" << gettime() - start << endl;

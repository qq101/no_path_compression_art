...
static art_leaf* make_leaf(uint8_t *key, unsigned key_len, uintptr_t value) {
...
bool leafMatches(Node* leaf, uint8_t key[], unsigned keyLength, unsigned depth, unsigned maxKeyLength) {
   ...
   art_leaf *l = (art_leaf*)getLeafValue(leaf);
   uint8_t leafKey[l->key_len];
   loadKey(l, leafKey);
...
uintptr_t lookup(Node* n, uint8_t* key, unsigned keyLength, unsigned maxKeyLength, unsigned depth = 0) {
   // Lookup the key
   if (n == NULL)
      return 0;
...
next:
      ...
      if (isLeaf(n)) {
         art_leaf *l = (art_leaf*)getLeafValue(n);
...
// Check if we are updating an existing value
      ...
      *nodeRef = newNode;
      ...
      if (existingKey[depth] == key[depth])
         ...
         insertNode4(newNode, nodeRef, key[depth], makeLeaf((uintptr_t)make_leaf(key, maxKeyLength, value)));

...
static art_leaf* make_leaf(uint8_t *key, int key_len, uintptr_t value) {
...
Node** findChildPort(Node* n, uint8_t keyByte, uintptr_t* port) {
   ...
   case NodeType4: {
      ...
      for (unsigned i = 0; i < node->count; i++) {
         if ((keyByte == '/' ) && (node->key[0] == '\0') && (flag_4 == 0))
         {
            *port = (uintptr_t)(((art_leaf*)(getLeafValue(node->child[0])))->value);
            flag_4 = 1;
         }
   ...
   case NodeType16: {
      ...
      int flag_16 = 0;
      if ((keyByte == '/' ) && (node->key[0] == '\0') && (flag_16 == 0))
      {
         *port = (uintptr_t)(((art_leaf*)(getLeafValue(node->child[0])))->value);
         flag_16 = 1;
      }
   ...
   case NodeType48: {
      ...
      int i0 = 0;
      int flag_48 = 0;
      i0 = node->childIndex['\0'];
      if (i0 && (keyByte == '/') && (flag_48 == 0))
      {
         *port = (uintptr_t)(((art_leaf*)(getLeafValue(node->child[i0 - 1])))->value);
         flag_48 = 1;
      }

      i = node->childIndex[keyByte];
      if (i)
   ...
   case NodeType256: {
      ...
      int flag_256 = 0;
      if (node->child['\0'] && (keyByte == '/') && (flag_256 == 0))
      {
         *port = (uintptr_t)(((art_leaf*)(getLeafValue(node->child['\0'])))->value);
         flag_256 = 1;
      }
...
bool leafMatches(Node* leaf, uint8_t key[], unsigned keyLength, unsigned depth, unsigned maxKeyLength) {
   ...
   if (depth != keyLength) {
      ...
      loadKey((art_leaf*)getLeafValue(leaf), leafKey);
...
unsigned prefixMismatch(Node* node, uint8_t key[], unsigned depth, unsigned maxKeyLength) {
   ...
   if (node->prefixLength > maxPrefixLength) {
      ...
      loadKey((art_leaf*)getLeafValue(minimum(node)), minKey);
...
uintptr_t lookup(Node* node, uint8_t key[], unsigned keyLength, unsigned depth, unsigned maxKeyLength) {
   ...
   while (node != NULL)
   {
      if (isLeaf(node))
      {
         if (depth == keyLength) // No check required
            return ((art_leaf*)getLeafValue(node))->value;

         if (depth != keyLength)
         {
            // Check leaf
            uint8_t leafKey[maxKeyLength];
            loadKey((art_leaf*)getLeafValue(node), leafKey);
            art_leaf * l = (art_leaf*)getLeafValue(node);
            int tmp_key = l->key_len - 1;
            if ((l-> key[tmp_key] == '\0') && key[tmp_key] == '/')
            {
               if (memcmp(l->key, key, tmp_key) == 0)
                  return l->value;
            }
            return port;
         }
      ...
      node = *findChildPort(node, key[depth], &port);
      depth++;
   }
   return NULL;
}
...
void insert(Node* node, Node** nodeRef, uint8_t key[], unsigned depth, uintptr_t value, unsigned maxKeyLength) {
   ...
   if (node == NULL) {
      *nodeRef = makeLeaf((uintptr_t)make_leaf(key, maxKeyLength, value));
      return;
   }
   ...
   if (isLeaf(node)) {
      ...
      art_leaf *l = (art_leaf*)getLeafValue(node);
      uint8_t existingKey[l->key_len];
      for (int i = 0; i < l->key_len; ++i)
      {
         existingKey[i] = l->key[i];
      }
      ...
      *nodeRef = newNode;
      ...
      insertNode4(newNode, nodeRef, key[depth + newPrefixLength], makeLeaf((uintptr_t)make_leaf(key, maxKeyLength, value)));
      return;
   }
   ...
   if (node->prefixLength) {
      ...
      if (mismatchPos != node->prefixLength) {
         ...
         *nodeRef = newNode;
         ...
         insertNode4(newNode, nodeRef, key[depth + mismatchPos], makeLeaf((uintptr_t)make_leaf(key, maxKeyLength, value)));
         return;
      }
      depth += node->prefixLength;
   }
   ...
   Node* newNode = makeLeaf((uintptr_t)make_leaf(key, maxKeyLength, value));
   switch (node->type) {
